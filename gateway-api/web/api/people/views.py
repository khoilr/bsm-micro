...
async def deletePersonByID(id: str):
    try:
        ...
        if deletedUser:
            return JSONResponse(
                status_code=200, content={"status": 200, "msg": f"Delete person with ID {id} succesfully"}
            )
        else:
            return JSONResponse(status_code=200, content={"status": 200, "msg": "Not found person to delete"})
    except Exception as e:
        return JSONResponse(status_code=400, content={"status": 400, "msg": e.__str__()})
